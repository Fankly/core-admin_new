...
import { PropType, defineComponent, ref, unref, computed } from 'vue'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const getMaxPercent = () => {
      const maxPercent = props.splitSet?.maxPercent
      if (maxPercent !== undefined) {
        return Math.min(maxPercent, 100 - props.splitSet?.minPercent || 0)
      }
      return 100 - (props.splitSet?.minPercent || 0)
    }
    ...
    const onClick = (): void => {
      if (!hasMoved.value) {
        ...
        const resetValue = Math.min(50, maxPercent - 5)
        percent.value = Math.max(resetValue, props.splitSet?.minPercent || 0) + 5
    ...
    const onMouseMove = (e: any): void => {
      ...
      if (active.value) {
        ...
        if (percents > (props.splitSet?.minPercent || 0) && percents < maxPercent) {
