...
import type { RangeVxeTableAutoColumnWidthOptions, RangeVxeTableRowClickMode, RangeVxeTableToolButton, RangeVxeTableTreeConfig } from './interface'
...
const props = withDefaults(
  defineProps<{
    ...
    /** 树形配置：透传到 vxe-grid treeConfig。lazy=true 时由组件内部包装 loadMethod 调用 requestApi。 */
    treeConfig?: RangeVxeTableTreeConfig
    /** 自定义树形懒加载取参（默认 { parentId: row[rowKey] }）。 */
    treeLoadParams?: (row: any) => Record<string, any>
    /** 预取的搜索枚举：setup 同步阶段注入 enumMap，跳过对应网络请求，解除与数据请求的串行。 */
