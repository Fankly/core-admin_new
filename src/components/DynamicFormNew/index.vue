      ...
      <el-row v-for="(row, rowIndex) in formRows" :key="rowIndex" :gutter="formConfig.gutter || 24">
        <template v-for="field in row">
          <el-col v-if="shouldShowField(field)" :key="field.prop" :span="field.span || Math.floor(24 / formConfig.colsPerRow)">
            <el-form-item
              :style="field.type === 'upload' ? { 'margin-bottom': 0 } : ''"
              :label="field.type === 'upload' ? '' : field.label"
              :prop="field.prop"
            >
              <!-- 输入框 -->
              <template v-if="field.type === 'input'">
                <el-input
                  v-if="appendData.includes(field.prop)"
                ...
                >
                  <template #append>
                    <el-button :disabled="field.disabled" class="el-icon-more" @click="handleSearch" />
                  </template>
                </el-input>
                <el-input
                  v-else
...
export default defineComponent({
  ...
  emits: ['submit', 'reset', 'field-change', 'input-append-handle', 'form-valid-change', 'upload-change', 'setValue', 'clearFieldValue'],
            ...
            try {
              ...
              const valueExists =
                ...
                (Array.isArray(currentValue)
                  ? currentValue.every((v) => options.some((opt) => opt.value === v))
