...
import { FormConfig, FormField } from '@/components/DynamicForm/interface'
...
export default defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    const dependencyRequestTokens: Record<string, number> = {}
    ...
    const isEmptyFieldValue = (value: any) => value === undefined || value === null || value === '' || (Array.isArray(value) && value.length === 0)

    const getDependencyValue = (dependency: string) =>
      Object.prototype.hasOwnProperty.call(formData, dependency) ? formData[dependency] : props.globalParams[dependency]

    // 处理字段值变化
    const handleFieldChange = async (prop: string, value: any, propagationPath = new Set<string>()): Promise<void> => {
      if (propagationPath.has(prop)) {
        const dependencyPath = [...propagationPath, prop].join(' -> ')
        console.error(`字段联动配置存在循环依赖：${dependencyPath}`)
        ElMessage.error(`字段联动配置存在循环依赖：${dependencyPath}，请联系管理员检查字段配置`)
        return
      }

      const currentPropagationPath = new Set(propagationPath)
      currentPropagationPath.add(prop)
      ...
      if (fieldDependencies[prop]) {
        ...
        for (const depField of dependentFields) {
          ...
          if (fieldConfig) {
            if (currentPropagationPath.has(depField)) {
              const dependencyPath = [...currentPropagationPath, depField].join(' -> ')
              console.error(`字段联动配置存在循环依赖：${dependencyPath}`)
              ElMessage.error(`字段联动配置存在循环依赖：${dependencyPath}，请联系管理员检查字段配置`)
              continue
            }

            const requestToken = (dependencyRequestTokens[depField] || 0) + 1
            dependencyRequestTokens[depField] = requestToken
            try {
              // 设置加载状态
              ...
              const hasMissingDependency = dependencies.some((dependency) => isEmptyFieldValue(getDependencyValue(dependency)))

              if (!hasMissingDependency) {
                if (fieldConfig.loadOptions) {
                  // 调用加载选项的函数
                  if (dependencies.length > 0) {
                    responseData = await fieldConfig.loadOptions(value, formData, dependencies as string[])
                  } else {
                    responseData = await fieldConfig.loadOptions(value, formData)
                  }
                }
                ...
                dependencies.forEach((dependency) => {
                  params[dependency] = getDependencyValue(dependency)
                })
                if (dependencies.length > 0) {
                  params['parentCode'] = getDependencyValue(dependencies[0])
                }

                if (fieldConfig.dyff && fieldConfig.ggdm && dependencies.length === 1) {
                ...
                if (fieldConfig.dyff && !fieldConfig.ggdm && dependencies.length > 0) {
                  const resData = await baseService.post(fieldConfig.dyff, params)
                  if (!resData.success) {
                    throw new Error(resData.msg || '接口请求失败')
                  }
              ...
              if (dependencyRequestTokens[depField] !== requestToken) continue

              const optionFieldTypes = ['select', 'radio', 'checkbox', 'treeSelect']
              if (!optionFieldTypes.includes(fieldConfig.type)) {
                ...
                if (isEmptyResponse) {
                  clearFieldValue(fieldConfig)
                  emit('clearFieldValue', fieldConfig)
                } else {
                  formData[depField] = responseData
                  emit('setValue', fieldConfig, responseData)
                }
                await handleFieldChange(depField, formData[depField], currentPropagationPath)
                continue
              }

              if (!isEmptyFieldValue(responseData) && !Array.isArray(responseData)) {
                throw new Error('接口返回的选项不是数组')
              }
              const options = Array.isArray(responseData) ? responseData : []
              fieldConfig.options = options

              // 后端选项以 code/name 为主，同时兼容 value/label 结构。
              const currentValue = formData[depField]
              const treeValueKey = fieldConfig.treeProps?.value || 'id'
              const treeChildrenKey = fieldConfig.treeProps?.children || 'children'
              const getOptionValue = (option: any) => {
                if (fieldConfig.type === 'treeSelect') return option?.[treeValueKey]
                return option?.code !== undefined ? option.code : option?.value
              }
              const containsOptionValue = (items: any[], targetValue: any): boolean =>
                items.some(
                  (option: any) =>
                    getOptionValue(option) === targetValue ||
                    (fieldConfig.type === 'treeSelect' &&
                      Array.isArray(option?.[treeChildrenKey]) &&
                      containsOptionValue(option[treeChildrenKey], targetValue))
                )
              const valueExists = Array.isArray(currentValue)
                ? currentValue.length > 0 && currentValue.every((current) => containsOptionValue(options, current))
                : containsOptionValue(options, currentValue)
              if (!valueExists) {
                clearFieldValue(fieldConfig)
                emit('clearFieldValue', fieldConfig)
                await handleFieldChange(depField, formData[depField], currentPropagationPath)
              ...
              ElMessage.error(`加载${fieldConfig.label || depField}失败：${error?.message || '未知错误'}，请重新选择后重试`)
            } finally {
              // 清除加载状态
              if (dependencyRequestTokens[depField] === requestToken) {
