...
<template>
  ...
  >
    <div class="import-excel">
      <el-form class="drawer-multiColumn-form" label-width="140px">
        <el-form-item :label="`${parameter.topName}：`">
          <el-button type="primary" @click="downloadTemp">点击下载</el-button>
        </el-form-item>
        <el-form-item :label="`${parameter.bottomName}：`">
          <el-upload
            ref="uploadRef"
            action="#"
            class="upload"
          ...
          >
            ...
            <template #tip>
              <slot name="tip">
                <div class="el-upload__tip">请上传 .xls , .xlsx, .et 标准格式文件，文件最大为 {{ parameter.fileSize }}M</div>
              </slot>
            </template>
          </el-upload>
        </el-form-item>
...
<script setup lang="ts" name="ImportExcelZx">
...
const uploadExcel = async (param: any) => {
  ...
  try {
    ...
    for (const key in fromData.value) {
      excelFormData.append(key, fromData.value[`${key}`])
    }
