...
export function getLabel(id: string, arr: any) {
  let label = null
  arr &&
    arr.forEach((element: any) => {
      if (String(element.code) === String(id)) {
        label = element.name
      }
    })
  return label
...
export function objectToQueryString(params: Record<string, any> | null | undefined) {
...
export function getAppId() {
  return sessionStorage.getItem('appId') || undefined
}

export function encrypt(value: any) {
  if (value == null || value === '') return null
  return encodeURIComponent(Base64.encode(String(value)))
}

export function treeToArray(data: any[]) {
  const result: any[] = []
  const traverse = (nodeOrArray: any) => {
    if (Array.isArray(nodeOrArray)) {
      nodeOrArray.forEach(traverse)
      return
    }
    if (!nodeOrArray) return
    result.push(nodeOrArray)
    if (nodeOrArray.children) traverse(nodeOrArray.children)
  }
  traverse(data)
  return result
}

export function traverseTree(data: any[] | undefined, callback: (node: any) => void, childKey = 'children') {
  const traverse = (nodeOrArray: any) => {
    if (Array.isArray(nodeOrArray)) {
      nodeOrArray.forEach(traverse)
      return
    ...
    if (!nodeOrArray) return
    callback(nodeOrArray)
    traverse(nodeOrArray[childKey])
  }
  traverse(data)
}

export function findParentPathIterative(tree: any[], targetId: any) {
  const stack = [{ node: { children: tree }, path: [] as any[] }]
  while (stack.length) {
    const { node, path } = stack.pop()!
    for (const child of node.children || []) {
      const currentPath = [...path, child.id]
      if (child.id === targetId) return currentPath
...
export function findNestedList(list: any[], matchKey: string, matchValue: any, nestedKey: string) {
  return (list || []).find((item) => item[matchKey] == matchValue)?.[nestedKey]
}

export function findTreeNodeObjectPath(treeRoot: any[], id: any, idKey = 'id', childKey = 'children') {
  const path: any[] = []
  const visit = (node: any): boolean => {
    path.push(node)
    if (node[idKey] === id) return true
...
export function findTreeNodePath(treeRoot: any[], id: any, idKey = 'id', childKey = 'children', resultKey?: string) {
  return findTreeNodeObjectPath(treeRoot, id, idKey, childKey).map((item) => item[resultKey || idKey])
}
...
export function decodeParams(encodedString: string) {
  const params = new URLSearchParams(atob(encodedString))
...
export const treeDataTranslate = translateTreeData

export function textToCopy(text: string) {
