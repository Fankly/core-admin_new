...
export class TableWidget {
  ...
  onCurrentPageChange(newCurrentPage) {
    return this.loadTableDataImpl(newCurrentPage, this.pageSize)
      .then(() => {
  ...
  onPageSizeChange(newPageSize) {
    ...
    this.currentPage = 1
    return this.loadTableDataImpl(1, newPageSize)
      .then(() => {
   ...
   */
  onSortChange({ prop, field, order }) {
    this.orderInfo.fieldName = prop || field
    this.orderInfo.asc = order === 'ascending' || order === 'asc'
    this.refreshTable()
  }
  /**
   * 获取每一行的index信息
  ...
  refreshTable(research = false, pageNum = undefined, showMsg = false) {
    ...
    if (Number.isInteger(pageNum) && pageNum !== this.currentPage) {
      return this.loadTableDataImpl(pageNum, this.pageSize, reload)
        .then((res) => {
          this.oldPage = this.currentPage = pageNum
          if (research && showMsg) ElMessage.success('查询成功')
        })
        .catch((e) => {
          this.currentPage = this.oldPage
        })
    } else {
      return this.loadTableDataImpl(this.currentPage, this.pageSize, true).catch((e) => {})
    }
  }
}
