...
<script setup lang="ts" name="ApplyOutboundConfirmBatchModal">
// 父组件传过来的参数
import { ref } from 'vue'
import { ElMessage } from 'element-plus'
import { VXETable } from 'vxe-table'
import { getProtypeTreeYearAll } from '@/api/common'
import { saveSqckProtypeConfirmData } from '@/api/config/sqckProtypeConfirmConfig'
...
const acceptParams = async (params: RowVO[]) => {
  ...
  try {
    ...
    formData.value.nd = params[0].nd
    // 回显
    isShowModal.value = true
    getTreeData(formData.value.nd)
  } finally {
    loading.value = false
  }
}
...
const handleSaveData = async () => {
  ...
  try {
    ...
    if (type === 'confirm') {
      loading.value = true
      // 选择数据
      const selectedIds = parameter.value.map((item) => item.protypeId)
      const res = await saveSqckProtypeConfirmData({
        protypeIds: selectedIds,
        ...formData.value,
