...
const formFields: FormField[] = [
...

  {
    ...
    label: '线路类型',
    type: 'select',
    options: lineTypeList.value as any
  },
...
const deleteRow = async (rows: any) => {
  ...
  const res = await sdxlxxRemoveApi(ids)
  if (res.success) {
...
const submitRow = async (rows: any) => {
  ...
  const res = await sdxlxxSubmit(ids)
  if (res.success) {
...
const handleSave = async (data: any) => {
  let res = await sdxlxxSaveApi(data)
  if (res.success) {
    clearSelect()
    proTableRef.value?.getTableList()
  } else {
...
const handleImport = () => {
  let newParmas = {}
  let tempApi: any = sdxlxxgetImportTemplate
  let importApi: any = sdxlxxImportExcel
...
const handleExport = async () => {
  ...
  try {
    if (proTableRef.value) {
      const params = proTableRef.value.searchParam
      const res: any = await sdxlxxExportExcel(params)
      const blob = new Blob([res])
      const blobUrl = window.URL.createObjectURL(blob)
      const exportFile = document.createElement('a')
      exportFile.style.display = 'none'
      exportFile.download = `输电线路信息-${new Date().getTime()}.xlsx`
...
const getPublicCodeData = async () => {
  ...
  const dydjRes = await getPublicData('DY_SDDYDJ_COM')
  if (zclxRes.success) {
  ...
  const lineTypeRes = await getPublicData('DY_XLLX_COM')
  if (zclxRes.success) {
    lineTypeList.value.length = 0
    lineTypeRes.data.forEach((v) => {
      lineTypeList.value.push({
        label: v.name,
        value: v.code
      })
    })
