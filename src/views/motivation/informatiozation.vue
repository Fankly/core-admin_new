...
<script lang="ts">
export default {
  name: '/motivation/informatiozation'
}
</script>
...
const deleteRow = async (rows: any) => {
  ...
  const res = await xxhxgxxRemoveApi(ids)
  if (res.success) {
...
const submitRow = async (rows: any) => {
  ...
  const res = await xxhxgxxSubmit(ids)
  if (res.success) {
...
const openModal = async (modeFlag: 'add' | 'edit' | 'view', row?: any) => {
  ...
  if (modeFlag !== 'add') {
    ...
    let res = await xxhxgxxGetInfoApi({ id: row.id })
    if (res.success) {
...
const handleSave = async (data: any) => {
  ...
  let res = await xxhxgxxSaveApi(data)
  if (res.success) {
    clearSelect()
    proTableRef.value?.getTableList()
  } else {
...
const handleImport = () => {
  let newParmas = {}
  let tempApi: any = xxhxgxxgetImportTemplate
  let importApi: any = xxhxgxxImportExcel
...
const handleExport = async () => {
  ...
  try {
    if (proTableRef.value) {
      const params = proTableRef.value.searchParam
      const res: any = await xxhxgxxExportExcel(params)
      const blob = new Blob([res])
      const blobUrl = window.URL.createObjectURL(blob)
      const exportFile = document.createElement('a')
      exportFile.style.display = 'none'
      exportFile.download = `信息化相关信息-${new Date().getTime()}.xlsx`
