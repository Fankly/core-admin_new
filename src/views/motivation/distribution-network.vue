...
<script lang="ts">
export default {
  name: '/motivation/distribution-network'
}
</script>
...
const deleteRow = async (rows: any) => {
  ...
  const res = await pdxlxxRemoveApi(ids)
  if (res.success) {
...
const submitRow = async (rows: any) => {
  ...
  const res = await pdxlxxSubmit(ids)
  if (res.success) {
...
const openModal = async (modeFlag: 'add' | 'edit' | 'view', row?: any) => {
  ...
  if (modeFlag !== 'add') {
    ...
    let res = await pdxlxxGetInfoApi({ id: row.id })
    if (res.success) {
...
const handleSave = async (data: any) => {
  ...
  let res = await pdxlxxSaveApi(data)
  if (res.success) {
    clearSelect()
    proTableRef.value?.getTableList()
  } else {
...
const handleImport = () => {
  let newParmas = {}
  let tempApi: any = pdxlxxgetImportTemplate
  let importApi: any = pdxlxxImportExcel
...
const handleExport = async () => {
  ...
  try {
    if (proTableRef.value) {
      const params = proTableRef.value.searchParam
      const res: any = await pdxlxxExportExcel(params)
      const blob = new Blob([res])
      const blobUrl = window.URL.createObjectURL(blob)
      const exportFile = document.createElement('a')
      exportFile.style.display = 'none'
      exportFile.download = `配电线路信息-${new Date().getTime()}.xlsx`
