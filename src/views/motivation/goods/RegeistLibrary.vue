...
<template>
  ...
  <FormModal ref="formModalRef" labelWidth="140px" title="注册库填报" :mode="mode" :fields="formFields" :data="formData"
...
const deleteRow = async (rows: any) => {
  ...
  const res = await zckxxRemove(ids)
  if (res.success) {
...
const submitRow = async (rows: any) => {
  ...
  const res = await zckxxSubmit(ids)
  if (res.success) {
...
const handleImport = () => {
  ...
  let newParmas = {}
  let tempApi: any = zckxxGetImportTemplate;
  let importApi: any = zckxxImportExcel;
...
const handleExport = async () => {
  ...
  try {
    if (proTableRef.value) {
      const params = proTableRef.value.searchParam
      const res: any = await zckxxExportExcel(params)
      const blob = new Blob([res])
      const blobUrl = window.URL.createObjectURL(blob)
      const exportFile = document.createElement('a')
      exportFile.style.display = 'none'
      exportFile.download = `注册库信息填报.xlsx`
