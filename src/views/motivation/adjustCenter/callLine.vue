...
<template>
  ...
  <FormModal ref="formModalRef" labelWidth="140px" title="通信线路信息填报" :mode="mode" :fields="formFields" :data="formData"
...
const deleteRow = async (rows: any) => {
  const ids = rows.filter((v: any) => v.status === '0').map((v: any) => v.id);
  await ElMessageBox.confirm('是否删除选中数据', {
    title: '提示',
    cancelButtonText: '取消',
    confirmButtonText: '确定'
  })
  const res = await txxlxxRemove(ids)
  if (res.success) {
...
const submitRow = async (rows: any) => {
  ...
  const res = await txxlxxSubmit(ids)
  if (res.success) {
...
const handleImport = () => {
  ...
  let newParmas = {}
  let tempApi: any = txxlxxGetImportTemplate;
  let importApi: any = txxlxxImportExcel;
...
const handleExport = async () => {
  ...
  try {
    if (proTableRef.value) {
      const params = proTableRef.value.searchParam
      const res: any = await txxlxxExportExcel(params)
      const blob = new Blob([res])
      const blobUrl = window.URL.createObjectURL(blob)
      const exportFile = document.createElement('a')
      exportFile.style.display = 'none'
      exportFile.download = `通信线路信息填报数据.xlsx`
...
const getPublicCodeData = async () => {
  await getLevelOne();
  const res: any = await getPublicData('DY_GLXLLX_COM')
  if (res.success) {
    lineType.value.length = 0;
    lineType.value.push(...res.data.map((v: any) => ({
      label: v.name,
      value: v.code
    })))
  } else {
