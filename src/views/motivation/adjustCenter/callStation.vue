...
<template>
  ...
  <FormModal ref="formModalRef" labelWidth="140px" title="通信站点信息填报" :mode="mode" :fields="formFields" :data="formData"
...
const deleteRow = async (rows: any) => {
  ...
  const res = await txzdxxRemove(ids)
  if (res.success) {
...
const submitRow = async (rows: any) => {
  ...
  const res = await txzdxxSubmit(ids)
  if (res.success) {
...
const handleImport = () => {
  ...
  let newParmas = {}
  let tempApi: any = txzdxxGetImportTemplate;
  let importApi: any = txzdxxImportExcel;
...
const handleExport = async () => {
  ...
  try {
    if (proTableRef.value) {
      const params = proTableRef.value.searchParam
      const res: any = await txzdxxExportExcel(params)
      const blob = new Blob([res])
      const blobUrl = window.URL.createObjectURL(blob)
      const exportFile = document.createElement('a')
      exportFile.style.display = 'none'
      exportFile.download = `通信站点信息填报数据.xlsx`
...
const getPublicCodeData = async () => {
  await getLevelOne();
  const res: any = await getPublicData('DY_ZDLX_COM');
  if (res.success) {
    stationType.value.length = 0;
    stationType.value.push(...res.data.map((v: any) => ({
      label: v.name,
      value: v.code
    })))
  } else {
