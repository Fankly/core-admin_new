  ...
  <vxe-modal v-bind="$attrs" :destroy-on-close="true" :loading="loading" @show="showHandle" @close="closeHandle" :title="props.title" v-model="showModal" show-zoom resize position="center">
    <div class="operation">
      <div class="operation-btn" v-if="toolButton">
        ...
        <el-button type="primary" plain @click="exportHandle">导 出</el-button>
      ...
      >
        <template v-for="item in tableMsg.columns" :key="item.columnKey">
          ...
          <vxe-column :formatter="formatterData" show-overflow header-align="center" align="right" v-else-if="item.eidt" min-width="160px" :field="item.columnKey" :title="item.columnValue" :edit-render="{ name: 'input', autofocus: '.my-input', autoselect: true }">
            <template #edit="{ row }">
              <input v-if="row?.id && !row?.leaf && item.eidt" @change="sumhandle(row, item.columnKey)" v-model="row[item.columnKey]" maxlength="20" v-number-input="tableMsg.xsws" class="my-input" />
            </template>
          </vxe-column>
...
import { computed, reactive, ref, defineExpose, defineEmits, withDefaults, defineProps, toRef } from "vue";
import { DwDataMsg, TableMsg } from "../../interface";
...
import { judgeSaved as cityjudgeSaved } from "@/api/fy/release/city";
...
export interface ModalProps {
  ...
  saveApi?: (params: any) => Promise<any> | null;
...
const dwMsg = reactive<DwDataMsg>({
  dwId: "",
  dwList: []
});
...
const sdHandle = async () => {
  if (props.sdzt && props.sdzt === "0") {
    ...
    let apiParams: any = {
      ProvinceReview: judgeSaved,
      CityReView: cityjudgeSaved
    };
    // 调用保存接口
    if (props.busiType === "YAP") {
      let saveRes = await apiParams[props.flag](params);
      if (!saveRes.data) {
  ...
  const updateRecordsLength = $table.getUpdateRecords().length;
  if (updateRecordsLength > 0) {
    const type = await VXETable.modal.confirm("存在数据未保存,是否继续进行审定？", "提示");
    if ("cancel" === type) {
      return;
    }
  }
  const type = await VXETable.modal.confirm("是否确定审定？", "提示", {
    status: "warning"
  });
  if (type === "confirm") {
    let api = props.sdApi;
