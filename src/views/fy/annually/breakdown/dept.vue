<template>
  <div class="container" v-show="isShowPage" v-loading="pageLoading">
    ...
    <el-tabs type="border-card" v-model="tabInfo.kmlx" @tab-click="tabClickHandle">
      ...
      <AnnuallyLayout
        :exportForDw="exportNdDataByBm"
        :exportForKm="exportNdDataByKm"
        v-if="isShowPage"
    ...
    >
      <template #opTypeLeft>
        <el-button type="primary" v-if="unitDescInfo.flag === 'edit'" plain @click="saveDataHandle"
          >保 存</el-button
        >
        <el-button type="primary" v-if="unitDescInfo.flag === 'edit'" plain @click="releaseHandle"
          >分解下达</el-button
        >
        <el-button
          ...
          >导 入</el-button
        >
        <el-button type="primary" plain v-permission="'EXPORT'" @click="unitDescExportHandle"
          >导 出</el-button
        >
      </template>
...
<script lang="ts">
export default {
  name: '/fy/annually/breakdown/dept'
}
</script>
...
import {
  ...
  exportData
} from '@/api/fy/annually/breakdown/dept'
...
const isDisabled = computed(
  () =>
    ...
    userInfo.status !== '5' ||
    userInfo.formStatus.bmfjStatus === '1' ||
    userInfo.formStatus.bmfjStatus === '2'
)
...
const unitDescHandle = (flag: string) => {
  ...
  unitDescInfo.title = flag === 'edit' ? '部门年度预算分解' : '部门年度预算分解-查看'
}
...
const releaseHandle = async () => {
  if (
    userInfo.formStatus &&
    (userInfo.formStatus.bmfjStatus === null || userInfo.formStatus.bmfjStatus === '')
  ) {
  ...
  if (type === 'confirm') {
    ...
    let isWorkFlowRes = await baseService.get(
      '/workflow/declare/getParamValue?paramKey=ND_DEPT_FJ_WORKFLOW'
    )
