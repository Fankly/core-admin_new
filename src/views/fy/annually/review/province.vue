      ...
      <el-tab-pane label="其他运营费用-主业" name="1">
        <div class="opeartion" v-if="userMsg.isShowPage">
          <div class="opeartion-left">
            <el-button type="primary" :disabled="isSaveDisabled" plain @click="saveTableHandle" v-permission="'SAVE'">保 存</el-button>
            ...
            <el-button type="primary" plain @click="exportTableDataHandle('zy')" v-permission="'EXPORT'">导 出 </el-button>
          </div>
      ...
      <el-tab-pane label="其他运营费用-农电" name="2">
        <div class="opeartion" v-if="userMsg.isShowPage">
          <div class="opeartion-left">
            ...
            <el-button type="primary" :disabled="isDisabled" plain @click="reviewHandle" v-permission="'AUDIT'">审 核 </el-button>
            <el-button type="primary" :disabled="isDisabled" plain @click="releaseHandle" v-permission="'RELEASE'"> 审定下达 </el-button>
            <el-button type="primary" plain @click="exportTableDataHandle('nd')" v-permission="'EXPORT'">导 出 </el-button>
          </div>
          <div class="opeartion-right">
            <span>当前状态：{{ userMsg?.statusInfo }}</span>
...
<script lang="ts">
export default {
  name: '/fy/annually/review/province'
}
</script>
...
import VXETable, { VxeColumnProps } from 'vxe-table'
import {
  getDataList,
  importDataByDw,
  getDwImportTemplate,
  ...
  exportForKm,
  saveDfj
} from '@/api/fy/annually/review/province'
...
const isSaveDisabled = computed(() => userMsg.status === '4')
...
const getDwData = async (specialorgid: string) => {
  ...
  if (res.success) {
    ...
    userMsg.status = res.data.status
...
const saveTableHandle = async () => {
  const $table = tabMsg.name === '1' ? zyTableRef.value : ndTableRef.value
  if ($table) {
    let resultData: any = {}
    // 获取表格录入的数据
      ...
      updateRecords.forEach((rowData) => {
        columnKeys?.forEach((item) => {
          if (!isNullOrUndefined(rowData[item.columnKey])) {
            resultData[rowData.id] = rowData[item.columnKey] ? rowData[item.columnKey] : '0'
          }
        })
      })
      let params = {
...
const sdDataHandle = (opType: string) => {
  ...
  dwMsg.opType = opType
  opType === 'edit' ? (dwMsg.title = '年度审定 - 单位审定') : (dwMsg.title = '年度审定 - 单位查看')
  dwMsg.dwId = record.dwId
  dwMsg.sdzt = record.sdzt
  dwMsg.busiId = record.dwDetailId
  dwMsg.operationFlag = opType
  opType === 'edit'
    ? (dwMsg.toolButton = ['save', 'import', 'export', 'sd', 'download', 'close'])
    : (dwMsg.toolButton = ['export', 'download', 'close'])
}
...
const isClickHeader = (columns: Columns[], field: string) => {
  let columnsData = columns.filter((item) => item.detail)
...
const headerCellClickHandle = ({ column }: any) => {
  ...
  if (isColumn) {
    ...
    opType === 'edit'
      ? (dwMsg.toolButton = ['save', 'import', 'export', 'sd', 'download', 'close'])
      : (dwMsg.toolButton = ['export', 'close', 'download'])
  }
}
...
const cellClickHandle = async ({ row, columnIndex }: any) => {
...
const sumhandle = (row: any, key: string) => {
  if (key.includes('_dfj')) {
    ...
    getGroupSummary(tableData, config, key, Number(tableMsg.xsws), 'leaf', 'id', columnKeys)
  }
}

const isEditHandle = (columns: Columns[], field: string, row: any) => {
  let columnsIndex = columns.findIndex((item: any) => item.eidt && !row.leaf && item.columnKey === field && !!row.id)
  return columnsIndex > -1
}

const cellStyle = ({ row, column, columnIndex }: any) => {
  const isColumn = isEditHandle(tableMsg.columns as Columns[], column.field, row)
