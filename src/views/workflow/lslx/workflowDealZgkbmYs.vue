...
<template>
  <div class="content">
    <div class="toolButtoon">
      <div class="left">
        <el-button size="mini" type="primary" plain @click="handlerClickBtn('pass')"> 通 过 </el-button>
        <el-button size="mini" type="primary" plain @click="handlerClickBtn('reject')"> 驳 回 </el-button>
...
import { zgkbmFinishWf, zgkbmReject } from '@/api/workflow/lslx/index'
...
const handlerClickBtn = (val: any) => {
  ...
  const api = val == 'pass' ? zgkbmFinishWf : zgkbmReject
  ElMessageBox.prompt('请输入审批意见', '提示', {
    ...
    .then(async (val: any) => {
      ...
      if (wfCodeString.value == 'ZGKBM_YSSHLC') {
