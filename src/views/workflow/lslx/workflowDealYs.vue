  ...
  <div class="content" ref="fromTableRef">
    <div class="toolButtoon">
      <div class="left">
        ...
        <el-button :disabled="pageInfo.loading" size="mini" type="primary" plain @click="handlerClickBtn('pass')"> 通 过 </el-button>
        <el-button :disabled="pageInfo.loading" size="mini" type="primary" plain @click="handlerClickBtn('reject')"> 驳 回 </el-button>
...
<script lang="ts">
export default {
  name: '/workflow/lslx/workflowDealYs'
}
</script>

<script setup lang="ts">
import { reactive, ref, onMounted } from 'vue'
import { getTableDataForYs, getDynamicColumnForYs } from '@/api/lslxJsc/index'
import { finishWfActivity, rejectActivity } from '@/api/workflow/lslx/index'
...
const handlerClickBtn = (val: any) => {
  ...
  const api = val == 'pass' ? finishWfActivity : rejectActivity
  ElMessageBox.prompt('请输入审批意见', '提示', {
    ...
    .then(async (val: any) => {
      pageInfo.loading = true
      if (wfCodeString.value == 'YSSHLC') {
        const res: any = await api({ ...params, spyj: val.value })
        if (res && res.success) {
          try {
            window.parent.Appframe.closePopWindow(window)
          } catch (e) {
            window.parent.postMessage(`已${text}`, '*')
          } finally {
            pageInfo.loading = false
          }
        } else {
