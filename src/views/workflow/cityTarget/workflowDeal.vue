...
import { finishWfActivity, rejectActivity } from '@/api/workflow/cityTarget/index'
...
const handlerClickBtn = (val: any) => {
  ...
  ElMessageBox.prompt('请输入审批意见', '提示', {
    ...
    .then(async (val: any) => {
      pageInfo.loading = true
      if (wfCodeString.value == 'WF_SYTC_MBZ_PHTZ') {
        ...
        if (res && res.success) {
          ...
          pageInfo.loading = false
          if (res.msg.includes('项目类型')) {
            const msg = res.msg.split('，')[0]
            ElMessage.error(`${msg}，请退回重新平衡调整！`)
          } else {
            ElMessage.error(res.msg)
          }
        }
