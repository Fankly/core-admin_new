...
import { finishActivity, finishZlActivity, getSearchData, rejectZlActivity, uploadCswj, deleteCswj } from '@/api/workflow/xm'
...
const submitDecWorkflowHandle = async () => {
  if (nodeCode.value == 'SG_CWB_ZZ') {
    const comfirmModal = await VXETable.modal.confirm('是否提交公司财务领导审核?', '提示', {
      confirmButtonText: '是',
      cancelButtonText: '否',
      zIndex: 3000
    })
    SCWLDSH.value = comfirmModal === 'confirm' ? 'Y' : 'N'
    if (comfirmModal) {
      submitFlow()
    }
  } else {
...
const submitWFCallback = async (nextPersonAndPath: string, wfData: WFData) => {
  ...
  let params =
    nodeCode.value == 'SG_CWB_ZZ'
      ? {
          ...spfrom,
          SCWLDSH: SCWLDSH.value
        }
      : { ...spfrom }

  let res: any
  if (wfCodeString.value === 'WF_NEWCBXQSHLC') {
