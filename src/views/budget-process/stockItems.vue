      ...
      <div v-loading="loading" class="process" element-loading-text="正在从服务器获取数据.....">
        <Split ratio="3/5">
          <template #one>
            <div class="left">
              <div class="search">
                <el-form :model="leftSearchData" label-position="left" :label-width="110">
                  <el-row :gutter="24">
                    <el-col :span="12">
                  ...
                  <el-row :gutter="24">
                    <el-col :span="12">
                      <el-form-item label="立项辞条名称：">
                        <el-input @change="getTreeData" v-model="leftSearchData.yssxct" placeholder=""></el-input>
                      </el-form-item>
                    </el-col>
                    <el-col :span="12">
                ...
                <el-form size="mini" label-position="right" :label-width="120">
                  <el-row :gutter="24">
                    ...
                    <el-col :span="8">
                      <el-form-item label="项目类型：">
                        <TreeSelect
                          @clearData="clearRightDataHandle"
                          ref="xmlxRef"
                          :is-leaf="false"
                          data-type="id"
                          :default-props="defaultProps.xmlxProps"
        ...
        <el-dialog title="历史查询" width="50%" v-model="isHistory">
          <el-table
            ...
            :header-cell-style="{ 'text-align': 'center' }"
          >
            <template v-for="item in historyColumns" :key="item.columnKey">
              <el-table-column
                show-overflow-tooltip
                :prop="item.columnKey"
                :formatter="(row: any, column: any, cellValue: any) => formatHandle(row, column, cellValue, item.columnKey)"
...
export default defineComponent({
  name: '/budget-process/stockItems'
})
</script>
...
const clearDataHandle = () => {
  leftSearchData.xmlx.length = 0
  getTreeData()
}

const clearRightDataHandle = () => {
...
const getTreeData = () => {
  ...
  baseService.post('process43/getCtListNew/', params).then((res) => {
    if (res.success) {
      loading.value = false
...
const getProjectTypeData = () => {
  let startDate = new Date().getFullYear().toString()
  if (searchData.ND) {
    startDate = searchData.ND
  }
  loading.value = true
  baseService.get('protypeTree/getSubProtypeTree/').then((res) => {
    if (res.success) {
      loading.value = false
...
const getLeftXmlxData = (value: any) => {
  leftSearchData.xmlx = value
  getTreeData()
}
...
const searchHandle = () => {
  const gwxmbm = xmbmRef.value.array ? xmbmRef.value.array : ''
  multipleSelection.value = []
  toggleSelection('')
  loading.value = true
  baseService
    .post('process43/getPageNew/', {
      ...formData,
      gwxmbm,
      page: page.page,
      limit: page.limit
...
const exportSxHandle = () => {
  loading.value = true
  baseService
    .export('/process43/exportCtlistNew', {
      nd: formData.nd,
...
const exportHandle = () => {
  ...
  baseService
    ...
    .then((res: any) => {
      ...
      let url = window.URL.createObjectURL(blob)
      dom.href = url
      dom.download = '战略预算-项目预算事项主数据关联存量项目.xlsx'
      document.body.appendChild(dom)
      dom.click()
      document.body.removeChild(dom)
      window.URL.revokeObjectURL(url)
...
const resetHandle = () => {
  ...
  searchData.ND = new Date().getFullYear().toString()
  formData.xmlx = []
  formData.yjdw = []
  formData.ejdw = []
  formData.xmmc = ''
