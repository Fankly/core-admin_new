<template>
  <div class="accounts27" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="topBox">
      <div class="title">
        <div class="titleBox">
          <h2>合同签订稽核</h2>
    ...
    >
      <el-table-column width="160" fixed label="项目编号">
        <template #default="scope">
          <a style="cursor: pointer" @click="jump(scope.row.pspid)">{{ scope.row.pspid }}</a>
        </template>
      </el-table-column>
      <el-table-column width="160" fixed prop="post1" label="项目名称"></el-table-column>
      ...
      <el-table-column width="160" prop="qkjyjdw_name" label="一级单位"></el-table-column>
      <el-table-column width="160" prop="qkjejdw_name" label="二级单位"></el-table-column>
      ...
      <el-table-column label="稽核疑点数量">
        <el-table-column prop="jh_htqdjsx_sl" v-if="columnObj.htqdjsx" label="合同签订及时性异常" width="160"> </el-table-column>
        <el-table-column prop="jh_htqdgfx_sl" v-if="columnObj.htqdgfx" label="合同签订规范性异常" width="160"> </el-table-column>
        <el-table-column prop="jh_htlygfx_sl" v-if="columnObj.htlvgfx" label="合同履约规范性异常" width="160"> </el-table-column>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default defineComponent({
  name: '/budget-process/process-27',
  components: {
    projectType,
    affiliatedUnit,
    copyTextBox,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post('/process/process27/', params).then((res) => {
        if (res.success == true) {
          this.loading = false
    ...
    async exportBtn() {
      ...
      await baseService.export('/process/process27/export', params).then((res) => {
        this.loading = false
        const blob = res
        let dom = document.createElement('a')
        let url = window.URL.createObjectURL(blob)
        dom.href = url
        dom.download = '合同签订稽核.xlsx'
