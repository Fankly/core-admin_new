<template>
  <div class="accounts28CT1" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="title">
      <h2>开工、竣工及时性</h2>
    </div>
...
export default defineComponent({
  name: "/budget-process/process-28-1",
  setup() {
    return reactive({
      tableKey: 0,
      example: [],
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post("/process/process28/ct1", params).then((res) => {
        if (res.success == true) {
          this.loading = false;
