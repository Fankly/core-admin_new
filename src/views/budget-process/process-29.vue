<template>
  <div class="accounts29" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="topBox">
      <div class="title">
        <div class="titleBox">
          <h2>项目审计稽核</h2>
    ...
    >
      <el-table-column width="160" fixed prop="pspid" label="项目编号">
      ...
      <el-table-column label="稽核疑点数量">
        <el-table-column v-if="columnObj.jsssjsx" label="结算送审及时性异常（个）" width="160" prop="jh_jsssjsxz_sl"> </el-table-column>
        <el-table-column v-if="columnObj.jssjjsx" label="结算审计及时性异常（个）" width="160" prop="jh_jssjjsxz_sl"> </el-table-column>
        <el-table-column v-if="columnObj.sjhjlg" label="审计核减率高异常（个）" width="160" prop="jh_sjhjl_sl"> </el-table-column>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default defineComponent({
  name: '/budget-process/process-29',
  components: {
    projectType,
    affiliatedUnit,
    copyTextBox,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post('/process/process29/', params).then((res) => {
        if (res.success == true) {
          this.loading = false
    ...
    async exportBtn() {
      ...
      await baseService.export('/process/process29/export', params).then((res) => {
        this.loading = false
        const blob = res
        let dom = document.createElement('a')
        let url = window.URL.createObjectURL(blob)
        dom.href = url
        dom.download = '项目审计稽核.xlsx'
