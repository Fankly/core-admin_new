<template>
  <div class="accounts25" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="topBox">
      <div class="title">
        <div class="titleBox">
          <h2>项目储备与立项稽核</h2>
    ...
    >
      ...
      <el-table-column label="稽核疑点数量">
        <el-table-column width="160" label="立项及时性"></el-table-column>
      </el-table-column>
      <el-table-column width="160" prop="ysxdrq" label="预算下达日期"></el-table-column>
      <el-table-column width="160" prop="qkjlxrq" label="项目立项日期"></el-table-column>
      <el-table-column width="160" prop="jh_lxjsxz" label="立项及时性"></el-table-column>
    </el-table>
    <el-pagination
...
export default defineComponent({
  name: '/budget-process/process-25',
  components: {
    projectType,
    affiliatedUnit,
    copyTextBox,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post('/process/process25/', params).then((res) => {
        if (res.success == true) {
          this.loading = false
    ...
    async exportBtn() {
      ...
      await baseService.export('/process/process25/export', params).then((res) => {
        this.loading = false
        const blob = res
        let dom = document.createElement('a')
        let url = window.URL.createObjectURL(blob)
        dom.href = url
        dom.download = '项目储备与立项稽核.xlsx'
