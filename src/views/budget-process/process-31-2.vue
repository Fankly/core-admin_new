<template>
  <div class="accounts31CT2" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="title">
      <h2>暂估转资及时性</h2>
    </div>
...
export default defineComponent({
  name: "/budget-process/process-31-2",
  setup() {
    return reactive({
      example: [],
      tableKey: 0,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post("/process/process31/ct2", params).then((res) => {
        if (res.success == true) {
          this.loading = false;
