<template>
  <div class="process" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    ...
    >
      ...
      <el-table-column label="调整日期" prop="tztime" width="250" v-if="columnObj.tztime"></el-table-column>
      <el-table-column label="调整人" prop="sqr" width="250" v-if="columnObj.sqr"></el-table-column>
      <el-table-column label="调整类型" prop="operate" width="250" v-if="columnObj.operate"></el-table-column>
      <el-table-column label="PMS设备编号(调整前)" prop="oldsbbh" width="250" v-if="columnObj.oldsbbh"></el-table-column>
      <el-table-column label="ERP资产编号(调整前)" prop="oldzcbh " width="250" v-if="columnObj.oldzcbh"></el-table-column>
      <el-table-column label="PMS设备编号(调整后)" prop="newsbbh" width="250" v-if="columnObj.newsbbh"></el-table-column>
      <el-table-column label="ERP资产编号(调整后)" prop="newzcbh" width="250" v-if="columnObj.newzcbh"></el-table-column>
    </el-table>
    <el-pagination
...
export default defineComponent({
  name: '/budget-process/process-41',
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post('process39/', params).then((res) => {
        if (res.success) {
          this.loading = false
    ...
    exportBtn() {
      ...
      baseService.export('/process39/export', params).then((res) => {
        this.loading = false
        const blob = res
        let dom = document.createElement('a')
        let url = window.URL.createObjectURL(blob)
        dom.href = url
        dom.download = '项目关联设备资产调整记录.xlsx'
