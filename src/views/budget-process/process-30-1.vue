<template>
  <div class="accounts30CT1" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="title">
      <h2>资金支付及时性</h2>
    </div>
...
export default defineComponent({
  name: "/budget-process/process-30-1",
  setup() {
    return reactive({
      example: [],
      tableKey: 0,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post("/process/process30/ct1", params).then((res) => {
        if (res.success == true) {
          this.loading = false;
