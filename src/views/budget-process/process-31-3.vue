<template>
  <div class="accounts31CT3" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="title">
      <h2>正式转资及时性</h2>
    </div>
...
export default defineComponent({
  name: "/budget-process/process-31-3",
  setup() {
    return reactive({
      example: [],
      tableKey: 0,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post("/process/process31/ct3", params).then((res) => {
        if (res.success == true) {
          this.loading = false;
