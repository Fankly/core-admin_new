<template>
  <div class="accounts31CT4" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="title">
      <h2>暂估增资合理性</h2>
    </div>
...
export default defineComponent({
  name: "/budget-process/process-31-4",
  setup() {
    return reactive({
      example: [],
      tableKey: 0,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post("/process/process31/ct4", params).then((res) => {
        if (res.success == true) {
          this.loading = false;
