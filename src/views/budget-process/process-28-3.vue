<template>
  <div class="accounts28CT3" v-loading="loading" element-loading-text="正在从服务器获取数据.....">
    <div class="title">
      <h2>物资超欠供情况</h2>
    </div>
...
export default defineComponent({
  name: "/budget-process/process-28-3",
  setup() {
    return reactive({
      example: [],
      tableKey: 0,
  ...
  methods: {
    ...
    loadData() {
      ...
      baseService.post("/process/process28/ct3", params).then((res) => {
        if (res.success == true) {
          this.loading = false;
