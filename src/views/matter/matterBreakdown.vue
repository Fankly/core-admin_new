  ...
  <div class="table-box" v-if="isShowPage" v-loading="loading">
    <proTable @search="searchHandle" @reset="resetHandle" :data-callback="callBackHandle" :init-param="initParam" :request-api="getPageList" :request-auto="true" :search-col="4" :columns="tableColumns" ref="proTableRef">
      ...
      <template #tableHeader="scope">
        <el-button v-permission="'RELEASE'" size="mini" :disabled="!scope.isSelected" type="primary" plain @click="handleDisposing(scope.selectedList, 'EDIT')"> 预算分解下达 </el-button>
        <el-button v-permission="'VIEW'" size="mini" :disabled="!scope.isSelected" type="primary" plain @click="handleDisposing(scope.selectedList, 'VIEW')">预算查看 </el-button>
      </template>
      <template #headerButton>
        <el-tabs v-model="tabPosition" @tab-click="searchTableHandle">
          <el-tab-pane label="省专项" name="0"></el-tab-pane>
          <el-tab-pane label="市自主" name="1"></el-tab-pane>
          <el-tab-pane label="县自主" name="2"></el-tab-pane>
        </el-tabs>
      </template>
    </proTable>

    <DisposingDialog @closePage="handleClosePage" :baseMsgData="baseMsgData" :dialogData="diaLogData" :title="diaLogData.title"></DisposingDialog>
  </div>
  <!-- 权限选择 -->
...
import DisposingDialog from "@/views/matter/components/DisposingDialog/index.vue";
...
import { getYsfjListPage } from "@/api/matter/matterYsfj";
