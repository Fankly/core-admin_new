...
export const useAiSmartTaskAuditPage = () => {
  ...
  /** 已选条数：用于工具栏前置禁用与计数展示，值由 @selection-change 同步 */
  const selectedTaskCount = computed(() => selectedRows.value.length)
  const selectedTaskNames = computed(() => selectedRows.value.map((row) => String(row.taskName || '').trim()).filter(Boolean))

  /** 把任务名拼成确认文案里的可读片段，超出 max 条时收尾为「等 N 个任务」 */
  const formatTaskNames = (rows: SmartTaskAuditRow[], max = 3) => {
    const names = rows.map((row) => String(row.taskName || '').trim()).filter(Boolean)
    if (!names.length) return `${rows.length} 个任务`
    const shown = names.slice(0, max).join('、')
    return names.length > max ? `${shown} 等 ${names.length} 个任务` : shown
  }
  ...
  const deleteTaskHandle = async () => {
    ...
    const type = await VXETable.modal.confirm(`确认删除「${formatTaskNames(rows)}」吗？删除后任务及其评审结果不可恢复。`, '删除任务', {
      status: 'warning',
  ...
  const executeRedoTask = async (
    execute: (taskId: string) => ReturnType<typeof redoTask>,
    buildConfirmMessage: (taskName: string) => string,
    confirmTitle: string,
    ...
    const type = await VXETable.modal.confirm(buildConfirmMessage(formatTaskNames(rows)), confirmTitle, {
      status: 'warning',
  ...
  const redoTaskHandle = () =>
    executeRedoTask(
      redoTask,
      (taskName) => `确认按原规则重新执行「${taskName}」吗？本次结果将覆盖该任务现有的评审结论。`,
      '原规则重新执行',
      '任务已提交按原规则重新执行'
    )
  ...
  const redoTaskWithCompleteRuleHandle = () =>
    executeRedoTask(
      redoTaskWithCompleteRule,
      (taskName) => `确认补全规则并重新执行「${taskName}」吗？补全的规则会一并参与审核，本次结果将覆盖该任务现有的评审结论。`,
      '补全规则重新执行',
      '任务已提交补全规则并重新执行'
    )
