<template>
  <component
    ...
    v-html="display"
    @click="onHtmlClick"
    @keydown.enter="onHtmlKeyActivate"
    @keydown.space="onHtmlKeyActivate"
  ></component>
</template>
...
import {
  convertAuditHtmlLineBreaks,
  getMarkdownRenderer,
  handleAuditHtmlActionClick,
  looksLikeHtml,
  looksLikeMarkdown,
  normalizeAuditMarkdown
} from './auditDetailHelpers'
import type { AuditAttachViewPayload, AuditPriceViewPayload, AuditReviewTablePayload } from './auditDetailHelpers'
...
const emit = defineEmits<{
  ...
  /** 点击后端下发的 .gwPriceView / .materialNewestPriceView，透传命中的类型 */
  (event: 'price-view-navigate', payload: AuditPriceViewPayload): void
...
const start = (source: string) => {
  ...
  // 智能识别 Markdown；Markdown 渲染结果与后端下发的 HTML 均原样保留，只做换行转换。
  const useMarkdown = looksLikeMarkdown(source)
  isMarkdown.value = useMarkdown || looksLikeHtml(source)
  const markdownSource = useMarkdown ? normalizeAuditMarkdown(source) : source
  const renderedText = useMarkdown ? getMarkdownRenderer().render(markdownSource).trim() : source
  rawText = convertAuditHtmlLineBreaks(renderedText)
...
/** 委托点击 v-html 内：同源目录 / 附件预览 / 结构化评审表 / 价格查看跳转 */
const onHtmlClick = (event: MouseEvent) => {
  handleAuditHtmlActionClick(event, {
    ...
    onPriceViewNavigate: (payload) => emit('price-view-navigate', payload)
  })
}

/** 键盘激活（Enter / Space）同源目录或附件预览节点 */
const onHtmlKeyActivate = (event: KeyboardEvent) => {
  const handled = handleAuditHtmlActionClick(event, {
    onAttachPreview: (payload) => emit('attach-preview', payload),
    onReviewTableOpen: (payload) => emit('review-table-open', payload),
    onPriceViewNavigate: (payload) => emit('price-view-navigate', payload)
  })
...
<style scoped>
.typewriter :deep(.attachView),
.typewriter :deep(.gwPriceView),
.typewriter :deep(.materialNewestPriceView),
.typewriter :deep(#sourceDirectory),
.typewriter :deep([data-id='sourceDirectory']),
.typewriter :deep([data-action='sourceDirectory']) {
  cursor: pointer;
}

.typewriter :deep(.attachView:focus-visible),
.typewriter :deep(.gwPriceView:focus-visible),
.typewriter :deep(.materialNewestPriceView:focus-visible),
.typewriter :deep(#sourceDirectory:focus-visible),
.typewriter :deep([data-id='sourceDirectory']:focus-visible),
.typewriter :deep([data-action='sourceDirectory']:focus-visible) {
