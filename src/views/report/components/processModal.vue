    ...
    <div class="table">
      <vxe-table  ref="tableRef" :data="tableData" border stripe resizable align="center" header-align="center" show-overflow show-header-overflow :row-config="{ height: 32 }" height="100%">
        ...
        <vxe-colgroup title="当前执行速度">
          <vxe-column field="dqzxsd_mbz" title="目标值"></vxe-column>
          <vxe-column field="dqzxsd_bys" title="包金额"></vxe-column>
          <vxe-column field="dqzxsd_lxz" title="立项值"></vxe-column>
          <vxe-column field="dqzxsd_wcz" title="完成值"></vxe-column>
        </vxe-colgroup>
...
import { getProgressDataByType } from "@/api/report";
...
const exportHandle = () => {
  ...
  if ($table) {
    $table.exportData({
      sheetName: "成本性项目三率报表-按类型-进度指标",
      type: "xlsx",
      filename: "成本性项目三率报表-按类型-进度指标",
      useStyle: true
    });
  }
};
...
const showHandle = async () => {
  const res = await getProgressDataByType({
    dwId: props.clickTableData.dw_id,
    startDate: props.clickTableData.startDate,
    endDate: props.clickTableData.endDate,
    xmlxId: props.clickTableData.xmlx_id
  });
  if (res.success) {
