...
<template>
  <div class="container" v-show="pageInfo.isShowPage">
    <div class="container-operation" v-if="pageInfo.isShowPage" style="margin-bottom: 10px">
      <div class="left">
        <el-button :loading="gridOptions.loading" v-permission="'EXPAND'" type="primary" size="mini" plain @click="expandHandle">一键展开</el-button>
...
import { getDynamicColumnBySgbmNew, getStatDataBySgbmNew } from '@/api/report'
...
import { vxeExportGkHandle } from '@/utils/export'
...
const loadChildrenMethod = ({ row }: any) => {
  return new Promise((resolve: any) => {
    let api = getStatDataBySgbmNew
    if (!api) return
    let params: any = {
      ...searchForm,
      ...apiParams.value,
      busiType: 'LYG_SGBM_YS_STAT',
      parentId: row.id
    }
...
const exportHandle = () => {
  ...
  if ($table) {
    vxeExportGkHandle($table, `市管部门维度报表`, gridOptions.data)
  }
}
...
const searchHandle = async () => {
  gridOptions.loading = true
  const res = await getStatDataBySgbmNew({
    ...searchForm,
    ...apiParams.value,
    busiType: 'LYG_SGBM_YS_STAT',
    parentId: null
...
const getDynamicColumnHeader = async () => {
  ...
  gridOptions.loading = true
  const res = await getDynamicColumnBySgbmNew({
    ...searchForm,
    ...apiParams.value,
    busiType: 'LYG_SGBM_YS_STAT'
  })
...
const resetHandle = () => {
  ...
  searchForm.indicatorCodeList = ['MBZ', 'WCZ', 'WCL']
