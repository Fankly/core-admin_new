<script lang="ts">
export default {
  name: "/ygConfigData/components/MainTable"
};
</script>
...
<template>
  <div class="table-box">
    <vxe-table  :loading="mainLoading" ref="mainTableRef" :show-overflow="true" :row-config="{ height: 32, isCurrent: false }" :data="props.mainTableData" align="center" height="100%" border resizable>
      <vxe-column width="50" type="checkbox"/>
      <vxe-column width="50" title="序号" field="dispOrder"/>
      <vxe-column title="编码" field="configCode"/>
      <vxe-column title="名称" field="configName"/>
      <vxe-column  :formatter="formatterStatMethod" title="统计方式" field="statMethod"/>
      <vxe-column  :formatter="formatterStatMethod" title="指标类型" field="indicatorType"/>
      <vxe-column width="100" :formatter="formatterRrc" title="状态" field="recState"/>
      <vxe-column width="100" title="是否显示" field="isDisplay" :formatter="formatterStatus"/>
      <vxe-column width="100" :formatter="formatterStatus" title="是否末级节点" field="isleaf"/>
      <vxe-column  :formatter="formatterStatus" title="是否纳入单位统计" field="isDwStat"/>
      <vxe-column width="100" title="年度" field="nd"/>
    </vxe-table>
  </div>
</template>
