...
<script setup lang="ts" name="/lyg/projectProcessWarning/maintenance/index">
...
import maintModal from '@/views/lyg/projectProcessWarning/maintenance/components/maintModal/index.vue'
import { getPublicData } from '@/api/common'
import { yjyzPage, yjyzSave, yjyzRemove } from '@/api/lyg/index'
...
const tableColumns = reactive<any[]>([
  ...
  { type: 'index', width: 50, label: '序号' },
  {
    prop: 'yjhj',
    label: '预警环节',
    enum: () => getPublicData('LYG_YJYZ_YJHJ_COM'),
  ...
  { prop: 'bm', label: '编码' },
  { prop: 'yjsjzqTx', label: '提醒时间周期(天)' },
  { prop: 'yjsjzqYj', label: '预警时间周期(天)' },
  { prop: 'yjsjzq', label: '警告时间周期(天)' }
])
...
const handleBtn = async (selectedList: SelectedRow[], type: OperationType) => {
  ...
  const res = await yjyzRemove(idList)
