...
import { slbbList, slbbSave, slbbRemove } from '@/api/service/deptData/index'
...
const getYjdwEnum = async () => {
  if (props.specialOrgId) {
    ...
    const record = await getDataByParent('ZYBMXMLX_COM')
    if (record.success) {
...
const saveHandle = async () => {
  ...
  if ($table) {
    ...
    if (insertRecords.length !== 0 || updateRecords.length !== 0) {
      ...
      if (type === 'confirm') {
        const res = await slbbSave(params)
        if (res.success) {
