...
export const offlineReview = () => {
  ...
  const canShowReturnButton = computed(() => userInfo.value?.canShowReturnButton === true)
  ...
  const selectProject = async (project: any) => {
    ...
    showXxLhhsYj.value = selectedProject.value.fzbReason
    clickRow()
  ...
  const initParamLists = async () => {
    try {
      ...
      // 按钮权限
      const passedButtonPermissions = userInfo.value?.buttonPermissions
      if (!Array.isArray(passedButtonPermissions)) throw new Error('未获取到按钮权限')
      buttonPermissions.value = passedButtonPermissions
      store.commit('setPermissions', buttonPermissions.value)
  ...
  const autoReview = () => {
    ...
    ElMessageBox.confirm(`${text}`, '提示', {
      ...
      .then(async () => {
        ...
        const thRes = await saveOrUpdateTXmThxxb({ list: thList })
        console.log(thRes)
        if (res.success) {
          await getPageList()
