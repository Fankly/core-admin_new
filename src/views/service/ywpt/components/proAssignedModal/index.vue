  ...
  >
    ...
    >
      ...
      <template #tableHeader="scope">
        <el-button :disabled="!scope['isSelected']" size="mini" type="primary" plain @click="handleIAssigned(scope['selectedList'])">
          分 配
        </el-button>
      </template>
      <template #headerButton>
        <el-tabs>
          <el-tab-pane label="未分配项目清单" name="0"></el-tab-pane>
        </el-tabs>
      </template>
    </ProTable>
  </vxe-modal>
...
import { addExpertProjectLink, pageExpertUnLinkedXmInfo } from '@/api/service/jointReview'
...
const handleIAssigned = async (selectedList: any[]) => {
  ...
  if (type === 'confirm') {
    try {
      loading.value = true
      const xmIdList = selectedList.map((res) => res.xmId)
      const res = await addExpertProjectLink({
...
const getPageList = (params: any) => {
  ...
  return pageExpertUnLinkedXmInfo(params)
}
