...
<template>
  <div>
    <vxe-modal ref="dialogFormRef" v-model="isShowModel" :destroy-on-close="true" resize show-zoom fullscreen :title="formData?.title" :width="1200" :close-on-press-escape="true" @close="closeHandle" :loading="tableInfo.loading">
      <div class="modal_content" ref="fromTableRef">
        <div class="toolButtoon">
          <div class="left">
            ...
            <el-button v-if="formData?.title == '二下数据编制'" size="mini" type="primary" plain @click="saveHandle">保 存</el-button>
            <el-button v-if="formData?.title == '二下数据编制'" size="mini" type="primary" plain @click="handlePass">二下核定</el-button>
            <el-button v-if="formData?.title == '二下数据编制'" size="mini" type="primary" plain @click="unitDescImportHandle">导 入</el-button>
        ...
        <div class="formClass">
          <el-form label-suffix=" : " ref="ruleFormRef" label-width="140px" label-position="right" :model="rmarkData" :rules="rules" :hide-required-asterisk="false">
            <el-row :gutter="20">
              <el-form-item label="展示方式" prop="viewMethod" label-width="94px">
                <el-select style="width: 200px" v-model="rmarkData.viewMethod" placeholder="请选择展示方式" @change="getViewMethod">
              ...
              <el-form-item label="一下全省总规模" prop="yxqszgm">
                <input v-limit-input class="my-input" disabled v-model.trim="rmarkData.yxqszgm" placeholder="请输入一下全省总规模" style="width: 200px" />
              </el-form-item>
              <el-form-item label="一下省级统筹规模" prop="yxsjtcgm">
                <input v-limit-input class="my-input" disabled v-model.trim="rmarkData.yxsjtcgm" style="width: 200px" />
              </el-form-item>
              <el-form-item label="一下市域统筹规模" prop="yxsytcgm">
                <input v-limit-input class="my-input" disabled v-model.trim="rmarkData.yxsytcgm" style="width: 200px" />
              </el-form-item>
            </el-row>
          </el-form>
        ...
        <div class="table">
          ...
          >
            <template v-for="item in tableInfo.columns" :key="item.id">
              <vxe-column header-align="center" show-overflow="tooltip" fixed="left" v-if="item.children.length == 0" border tree-node width="280" :title="item.title" :field="item.field" align="left" />
              <vxe-colgroup v-else :title="item.title" header-align="center">
                <template v-for="grop in item.children" :key="grop.id">
                  <vxe-column v-if="grop.edit && formData?.title == '二下数据编制'" :width="grop.width" :formatter="formatterData" :title="grop.title" :field="grop.field" header-align="center" align="right" border :edit-render="{ name: 'input', autofocus: '.my-input', autoselect: true }">
                    <template #edit="{ row }">
                      <input v-limit-input class="my-input" @change="sumhandle(row, grop.field)" v-model="row[grop.field]" />
                    </template>
                  </vxe-column>
                  <vxe-column v-else :formatter="formatterData" :width="'200'" header-align="center" border :title="grop.title" :field="grop.field" align="right" />
                </template>
              </vxe-colgroup>
            </template>
          </vxe-table>
...
import { getTableDataForEx, getDynamicColumnForEx, saveExhdje, getImportTemplateForEx, importExhdje, exportForEx, getYxGmSumData, saveYxqszgm, exhd } from '@/api/lslxJsc/index'
...
const tableInfo = reactive<any>({
  ...
  cellStyle: ({ row, column }: any) => {
    ...
    if (props.formData?.title != '二下数据编制') {
      return {
        cursor: 'auto',
...
const sumhandle = (row: any, key: string) => {
  ...
  for (const code in row) {
    if (code.includes('exhdje') && code != 'hj_exhdje') {
      row['hj_exhdje'] = Number(row['hj_exhdje']) + Number(row[code] != null ? row[code] : 0)
    }
  }
  ...
  summaryValue(tableData, config, 'hj_exhdje', 6, 'id')
}
// 获取编辑数据表格
const getEditDataHandle = ({ column, row }: any) => {
  ...
  if (outIndex > -1) {
    const index = updateParams[outIndex].dwValueList.findIndex((item: any) => item.field === column.field)
    if (index > -1) {
...
const exportHandle1 = () => {
  const $table = tableRef.value
  vxeExportHandle($table, '二下预算核定数据表', tableInfo.tableData)
}
// 导出
const exportHandle = async () => {
  try {
    ...
    const fileName = '二下预算核定数据表'
    apiExportHandle(params, fileName, exportForEx)
...
const unitDescImportHandle = async () => {
  ...
  let tempApi: any = getImportTemplateForEx
  let importApi: any = importExhdje
  if (!importApi) return
  let params = {
    title: '二下数据编制',
  ...
  try {
    ruleFormRef.value.validate((valid: any) => {
      ...
      if (updateParams.length === 0) {
        ElMessage.warning('请编辑二下核定金额后再进行保存！')
        return
      }
      const text = updateParams.length === 0 ? '检测到没有编辑二下核定金额，是否确定保存？' : '请确定要保存的内容'
...
const getDataList = async () => {
  const api: any = getTableDataForEx
  tableInfo.loading = true
...
const getHeaderData = async (params: any) => {
  const api: any = getDynamicColumnForEx
  let res = await api(params)
  if (res.success) {
...
const handlePass = async () => {
  ElMessageBox.confirm('是否核定并发布二下数据？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  })
    .then(async () => {
      let res = await exhd(props.formData?.versionId)
      if (res.success) {
        ElMessage.success('二下核定成功！')
        emits('getPass', true)
        newData()
      } else {
        ElMessage.error(res.msg)
