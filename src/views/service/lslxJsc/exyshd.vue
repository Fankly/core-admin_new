...
<template>
  ...
  <reportingex ref="reportRef" @get-pass="getPass" :formData="formData" />
</template>
<script lang="ts">
export default {
  name: '/service/lslxJsc/exyshd'
}
</script>
...
import reportingex from '@/views/service/lslxJsc/components/reportingex.vue' //上报组件
...
const clickBtn = async (val: any) => {
  try {
    ...
    if (val.selectedList[0].status != '7' && val.label == 'AUDIT') {
      return ElMessage.warning(`状态不是“二上省核定”，不允许编制。`)
    }
    const params = {
