...
import { pageXmInfoForYs, exportXmInfoForYs, mbzlslxNckSubmit } from '@/api/lslxJsc/index'
...
const clickBtn = async (val: any) => {
  try {
    ...
    const isSubmit = val.selectedList.some(
      (itme: any) => itme.lslxNckStatus == '2' || itme.lslxNckStatus == '3'
    )
    if (isSubmit && val.label == 'PROSUBMIT') {
      return ElMessage.warning(`存在已提交或已核定的数据，无需重复提交。`)
    }
    if (val.label == 'PROEXPORT') {
      try {
        ...
        const fileName = '省级统筹项目拟出库数据表'
        apiExportHandle(params, fileName, exportXmInfoForYs)
      ...
      ElMessageBox.confirm(`是否确定提交？`, '提示', {
        ...
        .then(async () => {
          ...
          const api: any = mbzlslxNckSubmit
...
const tableColumns = reactive([
  ...
  { prop: 'lslxNckStatusName', label: '拟出库状态', width: '100' },
  {
    prop: 'xmbm',
    label: '项目编码',
    width: '180',
  ...
  },
  {
    ...
    label: '项目名称',
    width: '180',
    search: { el: 'input', order: 2,props:{maxlength:60} }
  },
  {
    prop: 'jhssnd',
    label: '计划实施年份',
