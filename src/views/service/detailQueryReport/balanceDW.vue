...
<template>
  <div class="container" v-show="isShowPage">
    <otherTable @reset="reset" ref="otherTableRef" :columnsTable="columns" :tableType="'5'"
      :file-name="'费用单位平衡调整明细查询报表'" />
  </div>
...
<script lang="tsx">
export default {
  name: "/service/detailQueryReport/balanceDW"
};
</script>
...
const columns = reactive<any>([
  ...
  },
  {
    ...
    search: {
      ...
      render: () => {
        return (
          <TreeSelect ref={proDwRef} check-strictly={true} onClearData={resetDwData} is-child-node={false} data={dwList.value} onSelectChange={(value: any) => selectedDwData(value, "dwIdList")} is-leaf={false} data-type="id" default-props={treeProps.dwListProps} node-key="id" />
        );
      }
    },
    isShow: false
