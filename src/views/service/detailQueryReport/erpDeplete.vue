...
<template>
  <div class="container" v-show="isShowPage">
    <otherTable @reset="reset" ref="otherTableRef" :columnsTable="columns" :tableType="'1'" :file-name="'ERP消耗明细查询报表'" />
  </div>
...
<script lang="tsx">
export default {
  name: '/service/detailQueryReport/erpDeplete'
}
</script>
<script setup lang="tsx">
import { onMounted, ref, reactive, nextTick, onActivated, watch } from 'vue'
import { getPublicData, getDeptData, getYjdwFromCm, getEjdwByYjdw } from '@/api/common' //公共代码
...
//   if (Array.isArray($table.searchParam.cbzx)) {
//     $table.searchParam.cbzx.length = 0
//   } else {
//     $table.searchParam.cbzx = ''
//   }
// }
...
const columns = reactive<any>([
  ...
  //           is-child-node={false}
  //           data={cbzxList.value}
  //           onSelectChange={(value: any) => selectedData(value, 'cbzx')}
  //           is-leaf={false}
  //           ref={proTypeRef}
  //           data-type="objCode"
  ...
  // { prop: 'jjzxName', label: '基金中心' },
  { prop: 'xmxh', label: '项目消耗' },
  { prop: 'fyxh', label: '费用消耗' },
  { prop: 'zxh', label: '总消耗' }
])
