    ...
    <div class="container">
            ...
            <Grid ref="gridRef" :gap="[10, 0]" :cols="3">
              <GridItem>
                <el-form-item prop="dwType">
                  ...
                  <div class="form">
                    <el-select v-model="searchForm.dwType" @change="handleDeptChange">
      ...
      <div class="main">
        ...
        >
          ...
          <vxe-column width="300" field="dwType" title="类型" />
        </vxe-table>
      </div>
    </div>
...
import { gcGetStaffPage, gcSaveOrUpdate, gcDelete } from '@/api/service/xmcs/index'
...
const getYjdwEnum = async () => {
  if (props.specialOrgId) {
    ...
    const record = await getDataByParent('ZXCSGCTYPE_COM')
    if (record.success) {
...
const changeEjdwHandle = async () => {
  if (!searchForm.dwType) return
  loading.value = true
  const res = await gcGetStaffPage({
    ...searchForm,
    page: 1,
    size: 100
  })
  if (res.success) {
    ...
    tableData.value.forEach((item: any) => {
      const dwType = deptList.value.filter((records: any) => records.code == item.dwType)
      item.dwType = dwType[0]?.name || ''
    })
    loading.value = false
  } else {
    ElMessage.error(res.msg)
...
const delHandle = async () => {
  ...
  if ($table) {
    ...
    if (type === 'confirm') {
      ...
      const res: any = await gcDelete({ ids: ids })
...
const saveHandle = async () => {
  ...
  if ($table) {
    ...
    if (insertRecords.length !== 0 || updateRecords.length !== 0) {
      ...
      if (type === 'confirm') {
        const res = await gcSaveOrUpdate(params)
        if (res.success) {
