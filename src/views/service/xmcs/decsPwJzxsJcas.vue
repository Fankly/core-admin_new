...
function createPrimarySheetLayout(importedData) {
  ...
  const quotaSummaryRows = {
    ...
    profit: {
      rowIndex: quotaStartRowIndex + 12,
      label: '利润',
      subLabel: '',
      palette: 'green'
    },
    laborDiff: {
      rowIndex: quotaStartRowIndex + 13,
      label: '编制基准期价差',
      subLabel: '人工价差',
      palette: 'orange'
    },
    machineDiff: {
      rowIndex: quotaStartRowIndex + 14,
      label: '',
      subLabel: '机械价差',
      palette: 'orange'
    },
    total: {
      rowIndex: quotaStartRowIndex + 15,
      label: '合计',
      subLabel: '',
      palette: 'orange'
    }
  ...
  importedData.forEach((item) => {
    ...
    const primarySheet = createSheetModel({
      ...
      mergeData: [
        ...
        {
          startRow: quotaSummaryRows.winterRain.rowIndex,
    ...
    ;[
      ...
      quotaSummaryRows.winterRain.rowIndex,
      quotaSummaryRows.specialArea.rowIndex,
      quotaSummaryRows.tooling.rowIndex,
      quotaSummaryRows.temporaryFac.rowIndex,
      quotaSummaryRows.migration.rowIndex,
    ...
    const quotaSummaryConfigs = [
      ...
      {
        // 冬雨季施工增加费：定额基价 × 费率1.463%
        row: q.winterRain,
        formula: `=IF(G${q.base.rowIndex + 1}="","",G${q.base.rowIndex + 1}*C${q.winterRain.rowIndex + 1})`,
        extraCells: { 2: 0.01463 }
      },
      {
        // 特殊地区施工增加费：定额基价 × 费率0.000%
        row: q.specialArea,
        formula: `=IF(G${q.base.rowIndex + 1}="","",G${q.base.rowIndex + 1}*C${q.specialArea.rowIndex + 1})`,
        extraCells: { 2: 0.0 }
      },
      {
        // 施工工具用具使用费：定额基价 × 费率1.449%
        row: q.tooling,
        formula: `=IF(G${q.base.rowIndex + 1}="","",G${q.base.rowIndex + 1}*C${q.tooling.rowIndex + 1})`,
        extraCells: { 2: 0.01668 }
      },
      {
        // 临时设施费：定额基价 × 费率0.000%
        row: q.temporaryFac,
        formula: `=IF(G${q.base.rowIndex + 1}="","",G${q.base.rowIndex + 1}*C${q.temporaryFac.rowIndex + 1})`,
        extraCells: { 2: 0.07196 }
      },
      {
        // 施工机构迁移费：定额基价 × 费率0.000%
        row: q.migration,
        formula: `=IF(G${q.base.rowIndex + 1}="","",G${q.base.rowIndex + 1}*C${q.migration.rowIndex + 1})`,
        extraCells: { 2: 0.01912 }
      },
      {
        // 安全文明施工费：定额基价 × 费率5.688%
        row: q.safety,
        formula: `=IF(G${q.base.rowIndex + 1}="","",G${q.base.rowIndex + 1}*C${q.safety.rowIndex + 1})`,
        extraCells: { 2: 0.11018 }
      },
      {
        // 社会保险费：人工小计 × 费率27.3% × 调整系数1.1
        ...
        extraCells: { 2: 0.273, 3: 1.12 }
      },
      {
        // 住房公积金：人工小计 × 费率12% × 调整系数1.1
        row: q.housingFund,
