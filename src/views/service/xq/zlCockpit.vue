<template>
  <div v-show="isShowPage" class="rental-requirement-collection" v-loading="loading">
    ...
    <div class="process-flow">
      <template v-for="(step, index) in processSteps" :key="step.appId">
        <ProcessStep  :step="step" />
        <ProcessArrow v-if="index < processSteps.length - 1" />
      </template>
    </div>
...
<script lang="ts">
export default {
  name: "/service/xq/zlCockpit"
};
</script>

<script setup lang="ts">
import { getCurrentInstance, onMounted, ref } from "vue";
import { getProcessSteps, getTableConfig, getTableData } from "@/views/service/xq/process/api/index.js";
import DataTable from "@/views/service/xq/process/components/DataTable.vue";
import ProcessArrow from "@/views/service/xq/process/components/ProcessArrow.vue";
import ProcessStep from "@/views/service/xq/process/components/ProcessStep.vue";
import SearchSection from "@/views/service/xq/process/components/SearchSection.vue";
...
.rental-requirement-collection::before {
  ...
  right: 0;
  bottom: 0;
  background: radial-gradient(circle at 20% 20%, color-mix(in srgb, var(--color-primary) 5%, transparent) 0%, transparent 50%), radial-gradient(circle at 80% 80%, color-mix(in srgb, var(--color-primary) 3%, transparent) 0%, transparent 50%);
  pointer-events: none;
  z-index: 1;
}
