<script lang="ts">
export default {
  name: '/service/xq/ZLRequireDetailSearchNew'
}
</script>
<script setup lang="ts">
import { getPublicData } from '@/api/common'
import { getDynamicSearchColumn, getDynamicTableByUser, zlxqmxcxNewExport, zlxqszySearchNew, zlxmlx } from '@/api/service/requirement'
...
import ZLRequireDetailSearchBasicOperation from '@/views/service/xq/components/ZLRequireDetailSearchBasicOperation/index.vue'
import ProjectTypeAdjustment from '@/views/service/xq/components/ProjectTypeAdjustment.vue'
...
const ZLRequireDetailSearchBasicOperationRef = ref()
const ProjectTypeAdjustmentRef = ref()
...
const processPush = async () => {
  ...
  if ($grid) {
    ...
    loading.value = true
    const getPageData = await zlxmlx({
      xmIds: idList.value.join(',')
    })
...
const exportHandle = () => {
  ...
  zlxqmxcxNewExport(params).then((res: any) => {
    const blob = res
...
const viewHandle = () => {
  ...
  if ($grid) {
    ...
    customParam.value.bmId = records[0].create_dep_id || ''
    customParam.value.dwId = records[0].dw_id || ''
...
// 项目信息补充
const inforReplenishHandle = () => {
  const $grid = gridRef.value
  if ($grid) {
    const records = $grid.getCheckboxRecords()
    if (records && records.length !== 1) {
      ElMessage.warning('请选择一条数据进行项目信息补充!')
      return
    }
    selectedData.value = records[0]
    ZLRequireDetailSearchBasicOperationRef.value.isShowModal = true
  }
}

// 项目类型调整
const typeAdjustmentHandle = () => {
  const $grid = gridRef.value
  if ($grid) {
    const records = $grid.getCheckboxRecords()
    if (records && !records.length) {
      ElMessage.warning('请选择一条数据进行项目类型调整!')
      return
    }
    ...
    xmIds.value = idList.join(',')
    ProjectTypeAdjustmentRef.value.isShowModal = true
  }
}
...
<template>
  <div class="main" v-if="isShowPage" v-loading="loading">
    <div class="opeartion">
      <div class="left">
        ...
        <el-button v-permission="'PUSH'" plain type="primary" size="mini" @click="processPush">推送</el-button>
        <el-button v-permission="'XMXXBC'" plain type="primary" size="mini" @click="inforReplenishHandle">项目信息补充</el-button>
        <el-button v-permission="'XMLXTZ'" plain type="primary" size="mini" @click="typeAdjustmentHandle">项目类型调整</el-button>
      </div>
      <div class="right">
          ...
          <template v-for="column in dynamciSearch" :key="column.code">
            <el-col :span="6" v-if="column.link">
              <el-form-item :label="column.name">
                ...
                />
                <el-input v-else v-model="searchDatas[column.code]" maxlength="127" style="width: 100%"></el-input>
              </el-form-item>
            </el-col>
          </template>
  ...
  <ZLRequireDetailSearchBasicOperation
    :selectedData="selectedData"
    :matterBasicMsg="matterBasicMsg"
    :userInfo="userInfo"
    ref="ZLRequireDetailSearchBasicOperationRef"
    @save-data="searchDataHandle"
  />
  <ProjectTypeAdjustment
    :selectedData="selectedData"
    :typeAdjustMsg="typeAdjustMsg"
    :userInfo="userInfo"
    :xmIds="xmIds"
    ref="ProjectTypeAdjustmentRef"
    @confirm-data="searchDataHandle"
  />
</template>
...
:deep(.daterange-date .el-range-separator) {
  width: 8% !important;
}
</style>
