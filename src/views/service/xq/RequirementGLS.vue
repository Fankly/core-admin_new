...
<script lang="ts">
export default {
  name: '/service/xq/RequirementGLS'
}
</script>
<script setup lang="ts">
import { getPublicData } from '@/api/common'
import { getAppMenu } from '@/api/menu/menuConfig'
import { exportXm, getDynamicSearchColumn, getDynamicTableByUser, getXqlrPage, getXqkglth } from '@/api/service/requirement'
...
const reSendBtnHandle = async () => {
  ...
  if ($grid) {
    const records = $grid.getCheckboxRecords()
    if (records && records.length === 0) return ElMessage.warning('请至少选择一条数据进行操作!')
    ...
    try {
      ...
      if (type === 'confirm') {
        loading.value = true
        const res = await getXqkglth({ ids: ids.value, dwId: userInfOther.value.org_id, bmId: userInfo.value.deptId })
...
<template>
  <div class="main" v-if="isShowPage" v-loading="loading">
    <div class="opeartion">
      <div class="left">
        <el-button v-permission="'RETURN'" plain type="primary" size="mini" @click="reSendBtnHandle">退 回</el-button>
