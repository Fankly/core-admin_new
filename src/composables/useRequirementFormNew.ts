...
export function useRequirementForm(businessType: BusinessType) {
  ...
  const getFormDatas = async (id: string, xmlx: string, userInfo: UserInfo) => {
    try {
      ...
      steps.value = [steps.value[0], ...tabColumnsData.data.filter((item: any) => item.stepName !== '关联合同')] // 过滤掉关联合同 tab 页签
      stepIds.value = []
  ...
  const setValue = (field: any, value: string) => {
    if (value) {
      globalParams.value[field.prop] = value
    }
  }
  return {
    formConfigs,
