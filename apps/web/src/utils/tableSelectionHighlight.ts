...
export const useElTableSelectionHighlight = (tableRef: Ref<ElTableInstance | null>) => {
  ...
  const debouncedApplyRowHighlights = debounce(() => {
    ...
    const tableData = (tableRef.value.data || []) as TableRowData[]
    const rows = tableRef.value.$el.querySelectorAll('.el-table__row')
    const rowKey = (tableRef.value.rowKey as string) || ''

    const selectedKeys = new Set(selectedRows.value.map((row) => (rowKey && row[rowKey] !== undefined ? row[rowKey] : getRowKey(row))))

    rows.forEach((row: Element, index: number) => {
      const rowData = tableData[index]
      if (!rowData) return

      const rowKeyValue = rowKey && rowData[rowKey] !== undefined ? rowData[rowKey] : getRowKey(rowData)
...
export const useVxeTableSelectionHighlight = (tableRef: Ref<VxeTableInstance | null>, mode: 'single' | 'multiple' = 'multiple') => {
  ensureStyleElement()
  ...
  const handleSelectionChange = (params: { selection?: TableRowData[]; checked?: boolean; records?: TableRowData[] }) => {
    if (!tableRef.value) return
    ...
    if (allSelected.length === 0 && params.checked === false && params.records && params.records.length > 0) {
      const deselectedKeys = params.records.map(getRowKey)
      allSelected = selectedRows.value.filter((row) => !deselectedKeys.includes(getRowKey(row)))
    }
